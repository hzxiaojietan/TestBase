...
public class NetManager {
    ...
    public OkHttpClient.Builder  getOkHttpBuilder(String token) {
        return new OkHttpClient.Builder()
                    .readTimeout(60, TimeUnit.SECONDS)
                    .addInterceptor(new FZHeaderInterceptor(token))
    ...
    public class FZHeaderInterceptor implements Interceptor {
        String token;
        public FZHeaderInterceptor(String token) {
            this.token = token;
        }

        public Response intercept(Chain chain) throws IOException {
            Request.Builder builder = chain.request().newBuilder();
            Map<String, String> headers = mNetConfig.getHeaders(token);
            if (headers != null) {
